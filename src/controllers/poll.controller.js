...
const createPoll = asyncHandler( async (req, res) => {
	...
	if (!title || !options || !Array.isArray(options) || options.length < 2)  throw new ApiError(400, "Poll must include title and atleast 2 options");
...
const closePoll = asyncHandler( async (req, res) => {
	...
	if (!poll.userID.equals(userID)) throw new ApiError(403, "Not allowed");

	if (!poll.open) throw new ApiError(400, "Poll already close");
	poll.open = false;
	await poll.save();
